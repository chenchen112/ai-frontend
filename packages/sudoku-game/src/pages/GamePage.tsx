...
const generateEmptyBoard = (): Cell[][] =>
  Array.from({ length: 9 }, () => Array.from({ length: 9 }, () => ({ value: null, fixed: false })));
    ...
    <div className="flex flex-col items-center space-y-6">
      ...
      <div className="grid grid-cols-10 gap-2 w-full max-w-md">
        {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
          <button key={num} className="h-12 text-lg" onClick={() => handleNumberInput(num)}>
            {num}
          </button>
        ))}
        <button variant="destructive" className="h-12 text-lg" onClick={handleClearCell}>
          清除
        </button>
      </div>
    </div>
